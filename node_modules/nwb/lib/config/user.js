...
function getUserConfig(args = {}, options = {}) {
  ...
  let userConfig = {};
  let configPath = _path2.default.resolve(args.config || _constants.CONFIG_FILE_NAME);
  ...
  let configFileExists = _fs2.default.existsSync(configPath);
...
function getProjectType(args = {}) {
  ...
  let report = new _UserConfigReport2.default({ configFileExists, configPath });
...
function processUserConfig({
  ...
  let {
    type, polyfill,
    babel, devServer, karma, npm, webpack: _webpack } = userConfig,
      unexpectedConfig = _objectWithoutProperties(userConfig, ['type', 'polyfill', 'babel', 'devServer', 'karma', 'npm', 'webpack']);

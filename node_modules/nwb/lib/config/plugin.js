...
function getPackagePlugins(cwd) {
  let pkg = require(_path2.default.join(cwd, 'package.json'));
 ...
 * they export.
 */
function getPluginConfig(args = {}, { cwd = process.cwd() } = {}) {
  let plugins = [];
  ...
  plugins.forEach(plugin => {
    let pluginModule = require(_resolve2.default.sync(plugin, { basedir: cwd }));

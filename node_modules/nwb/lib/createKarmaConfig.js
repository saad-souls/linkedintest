 ...
 */
function getKarmaPluginConfig({ codeCoverage = false, userConfig = {} } = {}) {
  let browsers = ['PhantomJS'];
  let frameworks = ['mocha'];
  let plugins = [require('karma-sourcemap-loader'), require('karma-webpack')];
...
function createKarmaConfig(args, buildConfig, pluginConfig, userConfig) {
  ...
  let karmaConfig = {
    ...
    karmaConfig = (0, _webpackMerge2.default)(karmaConfig, userKarma.extra);
  }

  // Finally, give the user a chance to do whatever they want with the generated
  // config.
  if ((0, _utils.typeOf)(userKarma.config) === 'function') {
    karmaConfig = userKarma.config(karmaConfig);
    if (!karmaConfig) {
      throw new _errors.UserError(`karma.config() in ${userConfig.path} didn't return anything - it must return the Karma config object.`);
    }
  }

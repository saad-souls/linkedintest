...
function checkDirectories(dirs, cb) {
  (0, _runSeries2.default)(dirs.map(dir => cb => _fsExtra2.default.stat(dir, (err, stats) => {
...
packages, options, cb) {
  ...
  let spinner = (0, _ora2.default)(`Installing ${joinAnd(packages)}`).start();
  let npm = (0, _crossSpawn2.default)('npm', npmArgs, { cwd, stdio: ['ignore', 'pipe', 'inherit'] });
  npm.on('close', code => {
    if (code !== 0) {
      spinner.fail();
 ...
 */
const replaceArrayMerge = exports.replaceArrayMerge = (0, _webpackMerge2.default)({ customizeArray(a, b, key) {
    return b;
  } });
